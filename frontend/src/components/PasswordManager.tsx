...
export function PasswordManager({ initialCredentials = DEMO_CREDENTIALS, onSave }: PasswordManagerProps) {
  ...
  const addCredential = () => {
    ...
    const newCred: Credential = { id: nextId, ...form };
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-5">
        ...
        >
          <span className="text-lg leading-none">+</span> Add credential
        </button>
      </div>
      ...
      <input
        ...
        className="w-full px-3 py-2 text-sm border border-gray-200 rounded-md mb-3 focus:outline-none focus:border-gray-400"
      />
      ...
      <div className="space-y-2 mb-6 max-h-[400px] overflow-y-auto">
        {filtered.length === 0 && (
          <p className="text-center text-sm text-gray-500 py-8">No credentials found</p>
        )}
        {filtered.map((cred) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-xs font-medium flex-shrink-0 ${catStyle.bg} ${catStyle.text}`}>
                  {catStyle.label}
                </div>
                ...
                <span className={`text-xs px-2 py-0.5 rounded-md ${CONFIDENCE_BADGE[cred.confidence]}`}>
                  {cred.confidence}
                </span>
                <svg
              ...
              {/* Expanded detail */}
              {isExpanded && (
                <div className="px-3 pb-3 border-t border-gray-100 mt-0 pt-3 space-y-2">
                  {/* Username row */}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <code className="flex-1 text-xs bg-gray-50 px-2 py-1.5 rounded-md truncate">
                      {isShown ? cred.password : maskPassword(cred.password)}
                    </code>
                    <button
                  ...
                  <div className="flex gap-2 pt-1">
                    <button
                      ...
                      className="text-xs text-red-500 hover:text-red-700 border border-red-200 hover:border-red-300 px-2.5 py-1 rounded-md"
                    >
                      Remove
                    </button>
                  </div>
      ...
      {showModal && (
        ...
        >
          <div className="bg-white rounded-xl border border-gray-200 p-5 w-80 shadow-lg">
            ...
            <input
              className="w-full mb-3 px-2.5 py-1.5 text-sm border border-gray-200 rounded-md focus:outline-none focus:border-gray-400"
              value={form.name}
            ...
            <input
              className="w-full mb-1 px-2.5 py-1.5 text-sm border border-gray-200 rounded-md focus:outline-none focus:border-gray-400"
              value={form.password}
              ...
              <div className="mb-3">
                <div className="flex gap-1 mt-1 mb-0.5">
                  {[1,2,3,4].map((i) => (
                    <div
                      key={i}
                      className={`h-0.5 flex-1 rounded-full ${i <= strength.score ? strength.color : 'bg-gray-200'}`}
                    />
                  ))}
                </div>
            ...
            <select
              className="w-full mb-3 px-2.5 py-1.5 text-sm border border-gray-200 rounded-md focus:outline-none"
              value={form.category}
            ...
            <select
              className="w-full mb-4 px-2.5 py-1.5 text-sm border border-gray-200 rounded-md focus:outline-none"
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-5 left-1/2 -translate-x-1/2 bg-white border border-gray-200 rounded-lg px-4 py-2 text-sm shadow-md z-50">
          {toast}
        </div>
      )}
    </div>
...
function EyeIcon() {
  return (
    <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
      <path d="M1 6.5S3.5 2 6.5 2 12 6.5 12 6.5 9.5 11 6.5 11 1 6.5 1 6.5z" stroke="currentColor" strokeWidth="1.1"/>
...
function EyeOffIcon() {
  return (
    <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
      <path d="M1 6.5S3.5 2 6.5 2 12 6.5 12 6.5 9.5 11 6.5 11 1 6.5 1 6.5z" stroke="currentColor" strokeWidth="1.1"/>
      <path d="M3 2.5L10 10.5" stroke="currentColor" strokeWidth="1.1" strokeLinecap="round"/>
    </svg>
  );
}
